...
\lstdefinelanguage{jmjp-ebnf}{% 
	alsoletter={^|=}% 
	,classoffset=0% 
	,morekeywords={% Format 5 per row in dictionary order for readability
		,actAnk,actChi,actDmk,actKkn,actKrg% 
		,actOya,actPon,actRch,actRon,actRsn% 
		,actTmg,actTsm,bambooTiles,charTiles,circleTiles% 
		,dice,diceRes,discActs,discTile,dora% 
		,dragonTiles,drawActs,drawTile,endHand,file% 
		,fname,frame,frameAct,frameCmt,frameFlow% 
		,frameHonba,frameId,frameNum,frameSeq,framesRecd% 
		,frameWind,furoAnk,furoChi,furoDmk,furoKan% 
		,furoKkn,furoPon,hand,kyoutak,lname%
		,match,matchDate,matchDayOfWk,matchNumInDay,matchNumInStage% 
		,matchPlace,matchTime,matchTP,name,numTiles% 
		...
		,ptRetn,ptRule,ptStart,ptTour,ptTourNew% 
		,ptTourPrev,ptTourPrsnl,ptTourTeam,realTiles,recerInfo% 
		,recerName,seat,startHand,strLit,strNative% 
		,strRoman,tileChiHand,tileChiIn,tileDmkHand,tileDmkIn% 
		,tileFrom,tileFuro,tileInHand,tileKknIn,tilePonHand% 
		,tilePonIn,tileTsumo,tourInfo,tourName,tourStage% 
		,tourYear,unknownTiles,version,windTiles,wordTiles% 
	}% 
	,classoffset=2% 
...
%%  %%  
	\begin{version}[% 
		...
		\added{}% 
			\item{}Added requirement for software to be able to parse all sorthand tokens in a supported version. % 
			\item{}Added functionality of file comments that would be ignored by parsers. % 
			\item{}(For future use) Added a new class and corresponding style of identifiers, for when some keywords becomes deprecated but still needs formatting in changelog. % 
		\changed{}% 
